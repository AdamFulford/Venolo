...
myTimer.begin(DACupdate, 666); // ~500Hz
...
//Serial.print(MIDI.getThruState());
}
...
freq3Current = map(analogRead(freq3_pot),0,constants::in_max,0,constants::in_max);
  ...
  freq2Current = midi_matrix[5][2];
}

//scale with non-linear coefficients:

  freq1Current = Scale(freq1Current,constants::in_max,constants::a_freq,constants::b_freq);
...
int LFO2freq3{};  //value added to freq3 from LFO2
 ...
 if(LFO2 < 2048){
 LFO2freq1 = ((LFO2 * 2 / constants::DAC_max) - 1) * (amp2 / constants::in_max) * freq1Current;
 LFO2freq3 = ((LFO2 * 2 / constants::DAC_max) - 1) * (amp2 / constants::in_max) * freq3Current;
 }
...
//check freq1Current has changed, if so calculate values for DACupdate()
  ...
  if (abs(freq1Current-freq1)>constants::freq1Tolerance){//if reading from pot exceeds tolerance
    ...
    freq1scaled = (freq1 - constants::in_min) * (constants::freq_max - constants::freq_min) / (constants::in_max - constants::in_min) + constants::freq_min;
...
//check freq2Current has changed, if so calculate values for DACupdate()
  ...
  freq2scaled = (freq2 - constants::in_min) * (constants::freq_max - constants::freq_min) / (constants::in_max - constants::in_min) + constants::freq_min;
  period2 = constants::samplerate/(freq2scaled/100);
  wave2Inc = float(constants::waveLength)/period2;
  }
...
//check freq3Current has changed, if so calculate values for DACupdate()
  ...
  freq3scaled = (freq3 - constants::in_min) * (constants::freq_max - constants::freq_min) / (constants::in_max - constants::in_min) + constants::freq_min;
  period3 = constants::samplerate/(freq3scaled/100);
  wave3Inc = float(constants::waveLength)/period3;
  }
...
shape1a = map(shape1a_last,0,constants::in_max,0,2);
...
shape1b = map(shape1b_last,0,constants::in_max,0,2);
}
...
shape2 = map(shape2_last,0,constants::in_max,0,2);
}
...
shape3 = map(shape3_last,0,constants::in_max,0,2);
}

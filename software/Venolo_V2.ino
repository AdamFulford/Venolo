...
//clock multipliers and dividers for each LFO:

int LFO1aMult{6};  //up
int LFO1aDiv{1};

int LFO1bMult{6}; //down
int LFO1bDiv{1};
...
//need new scale function here to have wide middle band
  ...
  if (abs(swayCurrent-sway)>constants::swayTolerance){//if reading from pot exceeds tolerance
    ...
    {
      //up
      LFO1aMult = constants::swayUpMults[map(sway,constants::in_min,constants::in_max,0,constants::numSwaySettings - 1)];
      LFO1aDiv = constants::swayUpDivs[map(sway,constants::in_min,constants::in_max,0,constants::numSwaySettings - 1)];

      //down
      LFO1bMult = constants::swayDwnMults[map(sway,constants::in_min,constants::in_max,0,constants::numSwaySettings - 1)];
      LFO1bDiv = constants::swayDwnDivs[map(sway,constants::in_min,constants::in_max,0,constants::numSwaySettings - 1)]; 
    }
...
void DACupdateMode2()
{
    ...
    if (t > (constants::waveLength / 3) - 1)
      t = 0; // resets at the end of every full LFO1 cycle!
...
//if we have overshot end of wavelength or global t = 0
   if( (t1 > (constants::waveLength - 1)) || (t == 0) ){    
    t1 = 0; //reset LFO
  ...
  if(t == 0){ //if undershoot due to accuracy of mult / div (i.e. global t = 0)
    t1 = 0; //reset LFO, remain going down
  }
 ...
 if(t % LFO3Div == 0){ 
  ...
  if(direction2 == constants::up){  //going up
   if (t2 > (constants::waveLength - 1)){
    t2 = 0;
    direction2 = constants::down;  //set direction to down
   }
  }
  
  if(direction2 == constants::down){   
   if (t2 > (constants::waveLength - 1)){  
    t2 = 0;
  ...
  if(direction3 == constants::up){  //going up
   if (t3 > (constants::waveLength - 1)){
    t3 = 0;
    direction3 = constants::down;  //set direction to down
   }
  }
  
  if(direction3 == constants::down){   
   if (t3 > (constants::waveLength - 1)){  
    t3 = 0;
...
if(direction3 == constants::up)
  wave3Num = t3;
else if (direction3 == constants::down)
  wave3Num = constants::waveLength - t3 - 1;
...
uint16_t PWM_L_value{map(dac2,constants::DAC_min,constants::DAC_max,constants::PWM_min,constants::PWM_max)};
uint16_t PWM_R_value{map(dac1,constants::DAC_min,constants::DAC_max,constants::PWM_min,constants::PWM_max)};
...
dac1 = map(dac1,constants::DAC_min,constants::DAC_max,constants::DAC_max_out,constants::DAC_min_out);
dac2 = map(dac2,constants::DAC_min,constants::DAC_max,constants::DAC_max_out,constants::DAC_min_out);

//check engage switch is on
if(IS_ON){   //write dac values to MCP4922
//write values to dac
  MCP4922_write(0, dac1);
  MCP4922_write(1, dac2);
